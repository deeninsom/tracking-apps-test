...
import { QueryTimerDTO, QueryTimerLocationDTO } from './timer.dto';
...
export class TimerController {
  ...
  @Get('getCordinate')
  async getLocation(
    @Query() query: QueryTimerLocationDTO,
    @Res() res: Response,
  ) {
    try {
      const data = await this.timerService.getDataLocation(query.user_id, query.date);
      return res.status(200).json({
        status: true,
        message: 'Berhasil menampilkan hasil timer',
        data,
